...
import { ActiveIndicator } from './DurationSwitch/ActiveIndicator'
...
export const DurationSwitcher = (props: FlexProps) => {
  const { getRadioProps, getRootProps } = useRadioGroup({ defaultValue: 'monthly' })
  return (
    <Box pos="relative">
      <AnimateSharedLayout>
        <Flex
          ...
          {...getRootProps(props)}
        >
          <RadioButton {...getRadioProps({ value: 'monthly' })}>Monthly</RadioButton>
          <RadioButton {...getRadioProps({ value: 'yearly' })}>Yearly</RadioButton>
        </Flex>
      </AnimateSharedLayout>
